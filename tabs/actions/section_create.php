...
try {
    ...
    $grade_level_raw = isset($_POST['grade_level']) ? trim($_POST['grade_level']) : '';
    ...
    $co_adviser_id = isset($_POST['co_adviser_id']) && $_POST['co_adviser_id'] !== '' ? intval($_POST['co_adviser_id']) : null;
    ...
    $checkStmt = $conn->prepare("SELECT section_id FROM sections WHERE section_name = ? AND grade_level = ? LIMIT 1");
    if (!$checkStmt) {
    ...
    if ($checkRes && $checkRes->num_rows > 0) {
        ...
        throw new Exception('A section with that name and grade level already exists (id: ' . $existing['section_id'] . ').');
    }
    $checkStmt->close();
