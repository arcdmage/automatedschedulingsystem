...
try {
    ...
    $special = isset($_POST['special']) ? trim($_POST['special']) : null;
    ...
    $sql = "UPDATE subjects
            SET subject_name = ?, special = ?, grade_level = ?, strand = ?
            WHERE subject_id = ?";
    ...
    $strand_bind = $strand !== '' ? $strand : null;

    // Bind as strings; allow null values
    $stmt->bind_param('ssssi', $subject_name, $special_bind, $grade_bind, $strand_bind, $subject_id);
    ...
    if ($stmt->affected_rows === 0) {
        ...
        echo json_encode(['success' => true, 'message' => 'No changes were made to the subject.']);
        exit;
    }

    echo json_encode(['success' => true, 'message' => 'Subject updated successfully.', 'subject_id' => $subject_id]);
    exit;
