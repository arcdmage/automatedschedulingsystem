...
try {
    ...
    if ($requirement_id <= 0) {
        throw new Exception('Invalid requirement_id: must be a positive integer');
    }
    ...
    if (count($pattern) === 0) {
        throw new Exception('Pattern cannot be empty - please select at least one time slot');
    }
    
    // Log what we're processing
    error_log("Pattern Save - Processing requirement_id: $requirement_id with " . count($pattern) . " slots");
    
    // Get the section_id from the requirement
    $section_query = "SELECT section_id FROM subject_requirements WHERE requirement_id = ?";
    ...
    if (!$section_result) {
        throw new Exception('Requirement not found. The subject may have been deleted.');
    }
    ...
    $validate_query = "SELECT section_id, is_break FROM time_slots WHERE time_slot_id = ?";
    $validate_stmt = $conn->prepare($validate_query);
    if (!$validate_stmt) {
        throw new Exception('Database prepare failed for time slot validation: ' . $conn->error);
    }
    ...
    $check_pattern_sql = "
      SELECT sp.requirement_id, sr.subject_id
    ...
    if (!$check_pattern_stmt) {
        throw new Exception('Failed to prepare pattern-overlap check: ' . $conn->error);
    }
    ...
    if (!$check_schedule_stmt) {
        throw new Exception('Failed to prepare schedule-overlap check: ' . $conn->error);
    }
    ...
    foreach ($pattern as $index => $slot) {
        ...
        if (!isset($slot['time_slot_id'])) {
            throw new Exception("Pattern slot #$index is missing 'time_slot_id'");
        }
        
        if (!isset($slot['day_of_week'])) {
            throw new Exception("Pattern slot #$index is missing 'day_of_week'");
        }
        ...
        if (!isset($day_map[$day_key])) {
            throw new Exception("Invalid day_of_week at pattern slot #$index: '$day_raw'");
        }
        ...
        if (!$validate_stmt->execute()) {
            throw new Exception('Failed to validate time slot: ' . $validate_stmt->error);
        }
        
        $validate_result = $validate_stmt->get_result()->fetch_assoc();
        
        if (!$validate_result) {
            throw new Exception("Time slot ID $time_slot_id does not exist. Please refresh the page.");
        }
        
        if ($validate_result['section_id'] != $section_id) {
            throw new Exception("Time slot ID $time_slot_id does not belong to this section. Please refresh the page and try again.");
        }
        
        // Prevent scheduling during break times
        if ((int)$validate_result['is_break'] === 1) {
            throw new Exception("Cannot schedule classes during break time (slot $time_slot_id)");
        }

        // CHECK for pattern overlap with other saved patterns (same section/day/slot)
        $check_pattern_stmt->bind_param("iisi", $section_id, $time_slot_id, $day_of_week, $requirement_id);
        if (!$check_pattern_stmt->execute()) {
            throw new Exception('Failed to check existing patterns: ' . $check_pattern_stmt->error);
        }
        $existing_pattern = $check_pattern_stmt->get_result()->fetch_assoc();
        if ($existing_pattern) {
            throw new Exception("Conflict: another subject already has a pattern for $day_of_week at this time slot (time_slot_id={$time_slot_id}).");
        }

        // CHECK for overlap with already created schedules (actual assignments)
        $check_schedule_stmt->bind_param("iis", $section_id, $time_slot_id, $day_of_week);
        if (!$check_schedule_stmt->execute()) {
            throw new Exception('Failed to check existing schedules: ' . $check_schedule_stmt->error);
        }
        $existing_schedule = $check_schedule_stmt->get_result()->fetch_assoc();
        if ($existing_schedule) {
            throw new Exception("Conflict: an existing scheduled subject is already set on $day_of_week for this time slot (time_slot_id={$time_slot_id}).");
        }
    }
    $validate_stmt->close();
    ...
    if (!$conn->begin_transaction()) {
        throw new Exception('Failed to start database transaction: ' . $conn->error);
    }
    ...
    try {
        ...
        $delete_stmt = $conn->prepare("DELETE FROM schedule_patterns WHERE requirement_id = ?");
        ...
        if (!$delete_stmt->execute()) {
            throw new Exception('Failed to delete existing patterns: ' . $delete_stmt->error);
        }
        ...
        // Insert new patterns
        $insert_stmt = $conn->prepare("INSERT INTO schedule_patterns (requirement_id, day_of_week, time_slot_id) VALUES (?, ?, ?)");
        ...
        foreach ($pattern as $slot) {
            $day_raw = trim((string)$slot['day_of_week']);
            $day_key = strtolower($day_raw);
            if (is_numeric($day_raw)) $day_key = (string) intval($day_raw);
            $day = $day_map[$day_key];
            $time_slot_id = intval($slot['time_slot_id']);
            
            $insert_stmt->bind_param("isi", $requirement_id, $day, $time_slot_id);
            
            if (!$insert_stmt->execute()) {
                throw new Exception("Failed to insert pattern for $day at slot $time_slot_id: " . $insert_stmt->error);
            }
